...
namespace BlackCat\Database\Packages\Reviews\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\Reviews\Definitions;
use BlackCat\Database\Packages\Reviews\Criteria;
use BlackCat\Database\Packages\Reviews\Dto\ReviewDto as Dto;
use BlackCat\Database\Packages\Reviews\Mapper\ReviewDtoMapper as RowMapper;
...
final class ReviewRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Reviews\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Reviews\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Reviews\Dto\ReviewDto|null */
    public function getByTenantIdAndBookIdAndUserId(int $tenantId, int $bookId, int $userId, bool $asDto = false): array|\BlackCat\Database\Packages\Reviews\Dto\ReviewDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'book_id' => $bookId, 'user_id' => $userId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Reviews\Mapper\ReviewDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndBookIdAndUserId(int $tenantId, int $bookId, int $userId): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'book_id') . ' = :uniq_book_id' . ' AND ' . 't.' . Ident::q($this->db, 'user_id') . ' = :uniq_user_id';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_book_id' => $bookId, 'uniq_user_id' => $userId ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndBookIdAndUserId(int $tenantId, int $bookId, int $userId) {
        $row = $this->getByTenantIdAndBookIdAndUserId($tenantId, $bookId, $userId);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Reviews\Dto\ReviewDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Reviews\Dto\ReviewDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Reviews\Dto\ReviewDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\Reviews\Dto\ReviewDto|null {
